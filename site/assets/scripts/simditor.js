...
var BlockquoteButton, BoldButton, Button, CodeButton, CodePopover, ColorButton, Formatter, HrButton, ImageButton, ImagePopover, IndentButton, InputManager, ItalicButton, Keystroke, LinkButton, LinkPopover, ListButton, MarkButton, OrderListButton, OutdentButton, Popover, Selection, Simditor, SourceButton, StrikethroughButton, TableButton, TitleButton, Toolbar, UnderlineButton, UndoManager, UnorderListButton, Util,
...
MarkButton = (function(superClass) {
  ...
  MarkButton.prototype.icon = 'magic';
  ...
  MarkButton.prototype.command = function() {
    var marked, nodes, range;
    range = this.editor.selection.getRange();
    if (range.collapsed) {
      return;
    }
    nodes = this.getRangeNode(range);
    marked = false;
    nodes.forEach((function(_this) {
      return function(ele) {
        ...
        if ($ele.is('mark')) {
          ...
          return marked = true;
        } else if (ele.nodeType === 3 && $ele.parent().is('mark')) {
          $ele = $ele.parent();
          if (!marked) {
            _this.editor.selection.save();
          }
    ...
    nodes.forEach(function(ele) {
      ...
      if ($ele.is('span[style]')) {
        return $ele.replaceWith($('<mark>').html($ele.html()));
      } else if (ele.nodeType === 3 && $ele.parent().is('span[style]')) {
        $ele = $ele.parent();
        return $ele.replaceWith($('<mark>').html($ele.html()));
      }
    });
  ...
  MarkButton.prototype.nextNode = function(node) {
  ...
  MarkButton.prototype.getRangeNode = function(range) {
    var endNode, node, rangeNodes;
